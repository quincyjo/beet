...
def model_bg2(video, operator, N=100):
...
def morph_openclose(image, kernel_size=3, iterations=1):
    ...
    morph_openc = cv2.morphologyEx(morph_close, cv2.MORPH_OPEN, kernel,
                                   iterations=iterations)
    return morph_openc
...
def cross(rect, width, height, pt0, pt1):
        ...
        Returns 1 if the points enter the rect, -1 if it leaves,
        or 0 if neither.
        """
        x0 = pt0[0]
        y0 = pt0[1]
        ...
        p0in = (rect[0] < x0 < rect[0] + width and
                rect[1] < y0 < rect[1] + height)
        p1in = (rect[0] < x1 < rect[0] + width and
                rect[1] < y1 < rect[1] + height)
