...
def sortFilesByTime(files):
    # Sort by second
    files.sort(key=lambda fileName: fileName.split('_')[1].split(':')[2],
               reverse=True)
    # Sort by minute
    files.sort(key=lambda fileName: fileName.split('_')[1].split(':')[1],
               reverse=True)
    # Sort by hour
    files.sort(key=lambda fileName: fileName.split('_')[1].split(':')[0],
               reverse=True)
